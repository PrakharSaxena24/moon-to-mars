...
| D8 | **Rename/move** | Project → **`ogasawara-rehearsal`** at `/Users/tanakai/aibos/ogasawara-rehearsal` (implementation step #1; **not yet performed**). |

> **This turn produced this plan only.** No engine/app/UI code was changed and the folder was not moved.
...
- **Schedule (15):** every `consumesInfo` is satisfied by a handoff at `atTime ≤ start`, produced by an already-finished task → no character ever holds `waiting`/`confused`. Verified across all 35 tasks / 14 handoffs.
...
- **Two clocks.** The 10-day frame keeps `DT = 0.25 day`. The representative day runs a **fine minute clock** (`MIN_DT = 5`, window 05:00→20:00) only when `segment === 'fishday'`. Fixed `MIN_DT` ⇒ deterministic.
...
- **Checkpoints (関所).** At defined moments — `cp_briefing`, `cp_predep`, `cp_relay` (on catch relay), `cp_precook` — `tick()` sets `paused=true` and returns. The player **inspects each member**: info held (with arrival time), what they wait on (+predicted idle), next task. Then **intervenes**:
...
### 9.1 Worked example — one day, three seeded faults → D→A
Fault **A** = no abort owner / GO signal; Fault **B** = cook-consult arrows never drawn (wrong-fish rework);
Fault **C** = tackle list sent morning-of on a slow channel (late).

| | Run 0 (gappy) | +A `setSafety` | +B draw cook arrows | +C tackle→night-before |
|---|---|---|---|---|
| **Raw total** | 51 | 86 | 97 | 100 |
| `clean`? | no | no | no (C remains) | **yes** |
| **Score / Grade** | **51 / D** | **86 / B** | **89 / B** *(capped)* | **100 / A** |
| **Efficiency** | **71%** | **96%** | **98%** | **100%** |

The **89/B cap at 97-raw** is the intended lesson: near-perfect content, one un-timed arrow, still not an A.
Every deduction carries a sentence + a `fixId` (e.g. *"Boat idle 30 min at 港 — `ic_target` scheduled 06:00,
needed by 05:30"*). Seeded `mulberry32` guarantees the same plan reproduces the same ledger.
...
### 12.1 Task #1 — the rename/move (⚠ nested git)
`…/moon_to_mars` carries its **own nested `.git`** (remote `origin → PrakharSaxena24/moon-to-mars.git`), and sits
inside the outer repo `…/jp2enkotoba`. Target `…/ogasawara-rehearsal` does not exist.
1. Commit/stash dirty state inside `moon_to_mars`.
2. `mv …/jp2enkotoba/moon_to_mars …/aibos/ogasawara-rehearsal` (plain move — the nested `.git` travels with it → history/origin/branch preserved).
3. Check `git -C …/jp2enkotoba ls-files moon_to_mars` and `.gitmodules`; if it was a tracked path/submodule, remove that entry in the outer repo and commit the deletion.
4. **No path edits needed** — `index.html` loads `./engine.js` etc. by relative path; reopening from the new location just works. Confirm `node ogasawara-rehearsal/verify.js`.
5. Update `CLAUDE.md` header / `README.md` / `<title>` to the new name. **GitHub repo name & Pages URL are out of MVP scope** (a separate `gh repo rename`).

### 12.2 Engine additions (`engine.js`)
- **Constants:** `CHEFS=3`; recompute `GUESTS = HEADCOUNT − STAFF − CHEFS = 13`. New tunables `MIN_DT=5`, `DAY_START_MIN=300`, `DAY_END_MIN=1200`, `IDLE_TOL=0`.
...
- **Tasks:** the ~35 `fishday` tasks from §5, each with optional `startMin`/`durMin`/`day:'fishday'`/`produces[]` alongside the existing day-based fields (absent → behaves as today).
...
### 12.6 Phased task list
0. **Move & baseline** — §12.1; `node verify.js` green at the new path.
1. **Data model** — constants, chef role + p09–p11, `fishday` tasks + `handoffs[]`; fix `verify.js` individuals count.
2. **Temporal engine** — tunables + `fishday` segment; pure helpers; `handoffTiming` detector + `fixHandoffs` + merge clause; minute-clock branch; score fold.
3. **Verify the new gradient** — `fishday` D→A block.
4. **Authoring UI** — lanes + blocks + arrow overlay; `buildCfg` folds timing/handoffs; i18n + CSS.
5. **Playback UI** — minute `#nowtag`; Efficiency/Idle readouts; checkpoint inspector + intervene.
6. **Close-out** — manual pass (gappy shows 🧭 waiting ⏳ at vessel + 🍳 idle → `fixHandoffs` → 0 idle, dinner on time); re-run verify.
...
## 14. Files
```
ogasawara-rehearsal/            (currently jp2enkotoba/moon_to_mars/ — move is task #1)
...
## 15. Status
**Concept: COMPLETE** (this document). Roles, duties, the canonical timed information flow, authoring UX, run
model, and scoring are specified and the perfect-plan schedule is consistency-verified (zero idle, all handoffs
resolve). **Implementation: NOT STARTED** — no engine/app/UI code changed and the folder not moved in the turn
that produced this plan. Next action on go: execute the Phase-0 rename/move, then the Phase-1→6 task list in §12.6.
