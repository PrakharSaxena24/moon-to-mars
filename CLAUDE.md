...
>
> **Standalone first-play tutorial (2026-07-21):** [`The Hikone Morning — Campaign-Aligned Tutorial`](docs/superpowers/plans/2026-07-21-hikone-tutorial.md). It is separate from the Ogasawara campaign but teaches the same deck → arrange → connect → Run → causal report → exact fix → rerun grammar. The locked cast is Watanabe-san (渡邊さん), Prakhar (プラカール), and Nishinaga (西永); Watanabe-san arrives in his Mercedes-Benz G-Class / Geländewagen (ゲレンデ), with generation and color intentionally unspecified. The Ryokan Izumi → Lake Biwa story is the physical execution evidence and payoff. Do not restore either the rejected slideshow or the unrelated direct-manipulation fishing game as the onboarding grammar.

## User
- **Prakhar Saxena** (`prakhar.saxena@aibos.co.jp`) — primary user / product owner.

## Current owner amendments (2026-07-15; supersede older implementation history)
...
- Plan First is **cluster-first**: Trip Frame **11**, Load & Board **10**, Outbound Voyage **11**,
  Arrival **12**, Operations **13**, Fishing Day **34**, Return **9**. These seven values are exact
  `scoreTrip.byBucket` projections and sum to 100; causal clustering never mints or reallocates points.
- **100/100 is mastery of the clean modeled plan.** It is not an employee appraisal and remains
  separate from real-execution readiness. Every player-facing help/fix-pack action is navigation-only;
  it must never write a scored repair. Player-facing progress has no competing A/B/C/D scale; legacy
  engine grade fields are compatibility/history only. Physical custody is authored manually at every
  route handover.
- Morning authoring has a versioned, validated local autosave with explicit Resume/New/JSON Export/
  Import. Opening a link never auto-resumes. Event-beat pacing is the default, with Full clock as a
  result-equivalent option. Day 3 exposes three score-equivalent food-information solutions with
  distinct time, coordination, and path-failure consequences.
...
**known perfect modeled plan scores 100 and is mastered** when every duty, task, gate, resource, custody
handover, and timed information handoff is sound; the separate Efficiency headline records wasted time.
Where the plan has a gap, characters **idle,
...
| D6 | **Scoring** | Perfect modeled plan = 100; every plan-cause point is explainable. A separate headline **Efficiency %** records wasted time/rework effects beside the mastery score. |
...
2. **Start or resume explicitly** — a fresh poster is always stable. Morning edits autosave locally, but
   only Resume or a validated JSON import applies prior authoring state.
3. **Read the clusters** — the pre-built 24-person, 10-day skeleton opens as seven exact score clusters.
   Each collapsed cluster shows earned/max, mastery state, and root count; expansion shows the causal
   roots, downstream evidence, and editor targets rather than dumping the entire plan.
4. **Author the plan** — assign duties and guest buddies; drop **task blocks**; check each physical item
   through every custody handover; **draw an arrow** for every information handoff. “Take me there” may
   focus the exact control but never authors the answer. Day 3 also offers direct, delegated, and redundant
   food-information routes as three valid trade-offs.
5. **Predict and Run** — in Practice/Challenge, commit an expected root cause and cite the plan evidence
   first. Characters execute deterministic plan data. **Event beats** skip empty clock intervals by default;
   **Full clock** shows every step. Checkpoints still pause for inspect/intervene.
6. **Review** — compare prediction with modeled evidence; read clean-plan mastery, separate Efficiency,
   execution-readiness facts, and the causally clustered fix-pack. Every root returns to its exact editor.
7. **Fix & re-run** — make the planning change and test the causal chain again. **100/100 + clean = modeled
   plan mastery**; 100% Efficiency and real-execution readiness remain separate outcomes.
...
- **Canonical departure = 07:00**, canonical portions = **13 guests + 5 organizer add-ons = 18** (3 organizers self-cater; at-sea crew on bento are counted among the served). Freeze §5 as the single 100-point mastery anchor; `verify.js` asserts it.
...
5. **The mastery-gate moment** — a single late arrow leaves the plan below 100 and names the unfinished root.
...
├─ app.js       wires PRS + i18n to the DOM (plan / run / report / Live / revise)
...
Cluster-first mastery refinement SHIPPED 2026-07-15 (§32).**
...
Headless-verified `node verify.js` **513/513 checks** (classic D→A gradient; the fishday temporal block — gappy
plan idleTotal 1450 person-min, fishday-day efficiency 68%; the rubric v1.0 constitution — 99 atoms Σ=100,
gappy trip 51/100 at 86% trip Efficiency, canonical 100/clean/mastered at 100%, arrow-draw the largest single fix
...
façade equality, purity; plus Teaching-MVP readiness, channel-feasibility, deterministic-scenario pins,
cluster/root conservation, 100=mastery bindings, and Day-3 strategy trade-offs). The separate UI quality
pass covers persistence safety, event pacing, manual custody authoring, navigation-only help, and exact
fault routing.
Prior DOM passes: **69-check Playwright E2E** (§20.8) + headless-Chrome screenshot passes for §21/§22 + a
19-check runtime smoke for §24. Next: the explicitly deferred §§31–32 scenarios/transfer simulation, Layers 3–4
...
- **The constitution re-derived (§4.1 ruling).** The W1 engine landed Σ=100 but changed two frozen tiers
  ...
  **99 atoms**. Seed gaps extended (load: jig case never assigned to the truck run + cabin list unshared;
  ...
  efficiency 83%** (superseding §23's narrated 54/D under the re-packed matrix). The route-correction pass
  then pinned **52/D · 86%**; the current mastery-audited seed is **51/100 · 86%** (§15/§32). True canonical (all
...
> **Historical ship state:** §32 supersedes this section's player-facing grade terminology with the
> unified 100-point mastery scale. Engine A/B/C/D references here are retained only as 2026-07-14 test history.
...
- **A curriculum above the existing modes.** `learningLevel` is persisted separately from `appMode`:
  ...
  **Challenge** opens Plan First, requires the same prediction, applies `comms-outage`, and conceals projected
  answers until the run has been observed. No level has a score-granting automatic repair. It focuses Fishing Day and layers one unavailable
...
- **Three distinct verdicts.** `scoreTrip(plan)` remains the 99-atom, seven-bucket, six-dimension plan-artifact
  receipt (Σ=100); **100/100 + clean is mastery of that modeled plan**. It is not a score of the learner as a
  person. `tripEfficiency(plan)` remains the separate effect/wasted-time measure.
  `criticalAssumptions(plan)` and `executionReadiness(plan)` add the real-execution lens with statuses
  ...
  a canonical 100/clean mastered plan is rehearsal-complete but remains pending confirmation until all five are resolved.
...
- **One deterministic contingency.** `SCENARIOS` exposes `normal` and `comms-outage`;
  ...
  a radio-resilient plan can still reach the canonical 100/clean mastered rehearsal result.
...
- **Historical ship gate (2026-07-14).** `node verify.js` passed **431/431** checks at Teaching-MVP shipment,
  including every prior anchor plus readiness
...
## 32. Cluster-first mastery, safe authoring, and meaningful alternatives (SHIPPED 2026-07-15)

This refinement makes the whole-trip planner legible without weakening the 100-point constitution. It
implements the owner's rule that the plan Score is the mastery measure: **100 means the modeled plan is
mastered; anything below 100 identifies unfinished modeled work.** “Mastery” here describes the authored
plan, never the employee or learner as a person, and never overrides the separate real-execution check.

- **Seven exact clusters.** `planClusters(plan)` is a pure causal projection of `scoreTrip(plan)`, ordered
  `frame · load · voyage · arrival · ops · fishday · return`, with fixed maxima
  **11 · 10 · 11 · 12 · 13 · 34 · 9 = 100**. Each collapsed cluster shows earned/max, Mastered/Needs work,
  and its root count. Expansion shows grouped root causes, affected entities, human consequence, and lost
  points. The cluster `earned/maxPts` values equal `scoreTrip.byBucket` exactly; root grouping never mints,
  drops, or reallocates a score atom.
- **Causal roots and exact editor targets.** Failed atoms, detailed readiness, and classic detectors are
  partitioned once into `rootIssues`. Proven downstream waits fold into their single upstream socket root;
  unrelated faults stay separate. Each root returns a JSON-safe `editorTarget` discriminated as `detector`,
  `budget`, `role`, `guest`, `manifest`, `task`, `handoff`, or `card`. **Open plan**, report-side **Take me
  there**, report stage markers, and **Take me to the next/first issue** use that target to open the correct
  cluster/day drawer and focus the exact receipt choice, budget control, role/buddy selector, custody item,
  task block, socket, or arrow. Reports show the first actionable issue row and place the remainder under
  “more issues”; every report row likewise carries a precise navigation target.
- **No score-granting assistance.** The former Auto-fix/Auto-arrange affordances are navigation-only. Learn
  may explain a cause; Practice/Challenge may reveal it after observation; no hint, cluster, report row, or
  route button mutates a scored plan. Engine helpers such as `applyDayFix` remain deterministic fixture/test
  APIs, not player-facing mastery awards. The learner must author the change.
- **Manual physical custody.** Load (`custody`), Arrival (`transferCustody`), and Return (`returnCustody`)
  drawers expose every required manifest item against every physical handover task. A checkbox writes that
  task's `carries[]`; the chain is sound only when each required item remains present through every stage.

### 32.1 The 100=mastery constitution audit

The inventory remains **99 atoms / 100 max points**. Requirements that previously survived only as an
unclean grade gate now debit an already existing atom, so a known modeled gap cannot display 100:

1. the existing 2-point `frame_hospital_shared` gate requires both the emergency-access distribution and
   `ic_ferry` distribution to Site Lead, Specialist, Chef, Logistics, and Safety;
2. `load_safety_hd_l_hold` requires a complete Tokyo outbound custody chain;
3. the existing Arrival Logistics execution lane requires complete Chichijima transfer custody;
4. the existing Return Site Lead lane requires complete reverse-route custody, while the Return Logistics
   lane also requires named ownership of the legacy `t_ship` ship-out/headcount duty;
5. each existing Voyage per-guest care atom fails when its buddy's care work overlaps another assignment;
6. required flex/standby tasks remain lane-exempt but gate the existing Load headcount, Voyage watch, or
   Fishing-Day sea-watch safety atom when missing, compressed, misassigned, or overlapping.

Broken dependency edges on any authorable day debit the owning existing execution-lane atom, so they cannot
leave a dirty plan at 100 and do not add a new score row.

No new row or point was added for these bindings. A whole-plan Mastered label requires `total === 100`,
`gate.clean === true`, every cluster at max, and zero cluster roots. `tripEfficiency` still answers whether
the modeled execution wastes time. `executionReadiness` still answers whether the real trip has all five
external confirmations. A clean 100 can therefore be **plan mastery / rehearsal complete / real execution
pending confirmations**.

Player-facing progress is earned/max plus Mastered/Needs work only. The engine's retained `grade` field and
historical A/B/C/D examples elsewhere in this file are compatibility/test history, not a second scale for
current UI or public rubric copy.

### 32.2 Versioned authoring sessions

Morning authoring is saved after a 500 ms debounce under `localStorage['prs_authoring_plan']` in envelope
`{kind:'ogasawara-rehearsal-plan', version:1, savedAt, state}`. The state is limited to the authored classic
choices, mission-control overrides, six day overrides (including `carries[]`), role seats, guest buddies,
and selected day. Guided Live preserves the Morning snapshot instead of saving its lesson staging.

- Opening a link always shows the stable poster; saved state is discovered but never auto-applied or run.
- **Resume** applies the validated record. **New rehearsal** asks for confirmation, clears current/legacy
  records, resets every authoring surface, and starts a fresh save.
- **Reset rehearsal** also asks for confirmation, resets every authoring surface, and lets normal autosave
  replace the stored state with the fresh plan.
- **Export** downloads the current v1 envelope. **Import** accepts at most 1 MiB and validates exact keys,
  ids, numeric bounds, roles/people/tasks/cards/items/channels, seat uniqueness, buddy load, handoff topology,
  and relay prerequisites. Unknown versions and hostile/prototype-polluting shapes fail closed.
- Import writes the fully validated candidate to storage before applying it. Parse, validation, quota, or
  security failure leaves both the visible plan and the previous saved record unchanged.

### 32.3 Run pacing

Authored runs default to **Event beats**. `advanceAuthoredClock` advances the deterministic engine until the
signature changes—a task starts/stalls/recovers, a handoff becomes due, a problem/checkpoint/banner appears,
or the run finishes—so empty clock steps are not animated. **Full clock** performs one ordinary `tick` per
visual beat. Pacing and speed are presentation controls: both modes reach the same schedule, evidence, score,
and result. Every new run resets to Event beats.

### 32.4 Three Day-3 food-information solutions

The controlled causal root is `ic_food → t_f_menu`. `applyDay3FoodStrategy` clears sibling recipes before
writing the selected topology, and `day3FoodStrategy` projects the same feasibility/schedule/cluster engines
used for scoring. Each valid choice clears the one root and can preserve **34/34 Fishing Day and 100/100
whole-plan mastery**, without bonus points for redundancy:

| Strategy | Authored route | Earliest arrival / margin | Work | Failure consequence |
|---|---|---:|---:|---|
| Direct and fast | Budget Lead → Chef, face-to-face | 04:45 / 15 min | 1 transmission | One path; no path-failure tolerance |
| Delegated relay | Budget Lead → Comms by radio → Chef by phone | 04:52 / 8 min | 2 transmissions, 1 relay | Shares responsibility; forwarding fails closed if intake has not arrived |
| Redundant paths | Budget Lead → Chef independently by radio + phone | 04:46 / 14 min | 2 transmissions | Either path may fail; losing both restores the same single causal root |

A degraded recipe retains its valid authored `strategyId`, while `topologyStrategyId`, `recipeComplete`,
and `degraded` report the path structure that actually survives; conflicting identity metadata fails closed.

### 32.5 Verification

`node verify.js` passes **513/513** deterministic checks. The added engine guardrails cover exact cluster
order and maxima, score/root conservation and partitioning, canonical zero-root mastery, target stability
and purity, cross-bucket custody attribution, every existing-atom mastery binding above, all three strategy
consequence vectors and failure cases, and EN/JA parity. The separate UI quality pass covers event-jump/
full-clock equivalence, versioned autosave/import rejection and atomicity, manual custody editing,
navigation-only help, and exact report-to-editor routing.
