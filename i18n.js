...
(function (global) {
  ...
  var STR = {
    en: {
      ...
      mcHint: 'Build the operating plan, not just the score. Budget envelopes show who can approve and how payment works; resources show whether cash, ice, fuel and food are usable before the task starts.',
      mcStatusOk: 'Budget plan is usable',
      mcStatusBad: function (n) { return n + ' budget/resource gap' + (n === 1 ? '' : 's'); },
      mcStatusSub: 'Money must be available to the person doing the work, in a method the island vendor accepts.',
    ...
    ja: {
      ...
      mcHint: '点数だけでなく、現場で動く運営計画を作る。予算枠は誰が承認し、どの支払手段で使えるかを示し、資源は現金・氷・燃料・食数がタスク開始前に使えるかを示す。',
      mcStatusOk: '予算計画は使用可能',
      mcStatusBad: function (n) { return '予算・資源ギャップ ' + n + ' 件'; },
      mcStatusSub: 'お金は、作業する人が、現地業者の受け付ける手段で使える必要があります。',
      mcPeople: '担当者',
      mcEnvelopes: '予算枠',
      mcResources: '資源',
      mcSpendDrills: '支出ドリル',
