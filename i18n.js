...
(function (global) {
  ...
  var STR = {
    en: {
      ...
      orgTitle: 'Org & roles',
      orgHint: 'Roles, deputies and authority at a glance.',
      ...
      gridDayTitle: function (d) { return d + ' — who does what · who tells whom (read-only)'; },
      gridReadonlyHint: 'Read-only timeline. Each row is a person, blocks are their tasks, arrows are information passed between them. This day is scored by the checklist above — only Day 3 is planned to the minute here.',
      gridCommsLbl: 'Information flow (read-only)',
      gridNoArrows: '✓ No cross-person info hand-offs on this day.',
      gridScoreNote: 'Read-only view — this day’s score comes from the checklist above.',
      gridUnstaffed: function (task) { return task + ' — nobody assigned'; },
    ...
    ja: {
      ...
      gridDayTitle: function (d) { return d + ' — 誰が何を・誰に伝えるか（閲覧のみ）'; },
      gridReadonlyHint: 'この日の閲覧用タイムライン。各行が担当者、ブロックがそのタスク、矢印が人から人への情報の受け渡し。この日の採点は上のチェックリストで行います（分刻みの計画は3日目のみ）。',
      gridCommsLbl: '情報の流れ（閲覧のみ）',
      gridNoArrows: '✓ この日は担当者間の情報受け渡しはありません。',
      gridScoreNote: '閲覧のみ — この日の採点は上のチェックリストで行います。',
      gridUnstaffed: function (task) { return task + ' — 担当者未割当'; },
