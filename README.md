# Project Rehearsal · 実行前シミュレーター

A ~1–2 minute browser app that **rehearses a project before you run it**. You set the
plan — roles, authority, information, budget and report routes — press **Run**, and a small
crew rehearses the real **小笠原 (Ogasawara) 10-day / 24-person** company event on a site
map. Where the plan has a gap, the characters **pile up and show why**: ❓ hesitating,
💬 endless huddle, ⏳ waiting on approval, 😣 exhausted, 🔥 in crisis. Every stall is
**computed from the plan, never random**. Fix the gaps, re-run toward 100.
...
1. **Read the plan** — the Ogasawara event is pre-built (canvas, org & roles, 10-day timeline).
   The team is **8 AIBOS staff who run it + 16 group-company guests** they host.
2. **Choose a day** — rehearse **Day 1 (Arrival)**, **Days 2–9 (Operations)**, or **Day 10 (Return)**
   one at a time, or the **whole trip**. Each day shows only its own design decisions and gap count.
3. **Close that day's gaps** — seven decisions ship intentionally missing (the demo opens gappy):
   ferry-time sharing (arrival), safety abort-authority, meals budget authority, illness report route,
   site-lead deputy/load (operations), cash reserve, and return shipping & headcount (return).
4. **Run that day** — watch the site map; gaps make crews pile up at a zone and show the reason.
5. **Review** — the day's grade + tasks, an overall 8-category trip score, per-person values, and a fix-pack.
...
teaching gradient is **headless-verified**: `node verify.js` asserts the gappy Ogasawara plan
grades **D**, each fix removes its detector and raises its category, all fixes reach a clean
**A (100)**, and the same seed reproduces the same score. Verified gradient:

```
gappy        D   9 /100   gaps=6
+setSafety   D  26 /100   gaps=5
+grantAuth   D  39 /100   gaps=4
+shareInfo   D  51 /100   gaps=3
+setReport   C  62 /100   gaps=2
+rebalance   B  89 /100   gaps=1
+fixReserve  A 100 /100   gaps=0
```
...
## 🗺️ Scope (MVP) / 範囲
This is the **MVP** the spec recommends starting from (§24): the guided Ogasawara rehearsal,
five+ problem detectors mapped to character behaviors, the 8-category score + individual
values, and the on-screen improvement pack. **Out of scope** (spec Phase 2/3): AI task/risk
generation, replay timeline, post-execution comparison, and Word/PDF/Excel export.
